...
namespace MahjongAlgorithmForLHZMJ
{
    /// <summary>
    /// 麻将通用算法
    /// </summary>
    public class MahjongGeneralAlgorithm
    {
        ...
        /// 是否可听
        /// </summary>
        /// <param name="voteRight"></param>
        /// <returns></returns>
        public static bool CheckVoteRight_Ting(byte voteRight)
        {
            return (voteRight & MahjongDef.gVoteRightMask_Ting) > 0;
        }
        /// <summary>
        /// 检查投票权限:是否可碰
        /// </summary>
        ...
        /// <param name="Ary"></param>
        /// <param name="card"></param>
        public static void RemoveAryAllParam(List<byte> Ary, byte card)
        {
            if (Ary == null || Ary.Count < 1 || card == MahjongDef.gInvalidMahjongValue)
            {
                return;
            }

            while (Ary.Contains(card))
            {
                Ary.Remove(card);
            }

            //Ary.RemoveAll(delegate (byte checkCard) { return checkCard == card; });//其实可有用这种写法，如果上面的写法运行出现问题，就用这种写法
        }
        ...
        /// <param name="vectorTingCard"></param>
        /// <returns></returns>
        public static bool CheckIfCanTingCardArrayForWHMJ(List<byte> srcAry, List<byte> vectorTingCard = null,bool ifSevenPair = true)
        {
            if (null != vectorTingCard)
            {
                vectorTingCard.Clear();
        ...
        /// </summary>
        /// <returns></returns>
        public static List<byte> GetCXQD()
        {
            List<byte> zi = new List<byte>();
            byte zi1 = 0x41;
            for (int i = 0; i < 4; i++)
        ...
        /// </summary>
        /// <returns></returns>
        public static List<byte> GetMLZJ()
        {
            List<byte> zi = new List<byte>();
            byte zi1 = 0x45;
            for (int i = 0; i < 4; i++)
        ...
        /// <returns></returns>
        public static int GetCardTypeNumFromCardAry(List<byte> cardAry)
        {
            int typeNum = 0;
            if (cardAry == null || cardAry.Count < 1)
            {
                return typeNum;
            }
            List<byte> cardAy = new List<byte>(cardAry);
            cardAy.Sort();
            int count = cardAy.Count;
            for (int i = 0; i < count; i++)
            {
                if (i == 0)
                {
        ...
        /// <param name="vectorCard"></param>
        /// <returns></returns>
        public static bool ChangeArrayTo7PairsNeedHun(List<byte> vectorCard, int hunNum)
        {
            int needHun = 0;
            if ((vectorCard.Count + hunNum) != 14)
            {
                return false;
            }
            if (GetCardTypeNumFromCardAry(vectorCard) > 7)//牌类型数大于7不可能组成7对
            {
                return false;
            }
            if (vectorCard == null)
            ...
            needHun = ChangeArrayToPairsNeedHun(vectorCard);


            //if(hunNum > 0 && hunNum %2==0)
            //{
            //    if(needHun==0)
            //    {
            //        isHunPaiGuiWei = true;
        ...
        /// <param name="vectorCard"></param>
        /// <returns></returns>
        private static int ChangeArrayToPairsNeedHun(List<byte> vectorCard)
        {
            int needHun = 0;
            if (vectorCard == null || vectorCard.Count < 1)
            {
                return needHun;
            }

            List<byte> cardList = new List<byte>(vectorCard);
            cardList.Sort();

            List<byte> cardAry = new List<byte>();
            for (int i = 0; i < cardList.Count; i++)
            {
                if (i != cardList.Count - 1 && cardList[i] == cardList[i + 1])
                {
                    for (int j = 0; j < cardList.Count; j++)
                    {
                        if (j != i && j != i + 1)
                        {
                            cardAry.Add(cardList[j]);
                        }
                    }
                    needHun = ChangeArrayToPairsNeedHun(cardAry);
                    return needHun;
                }
            }
        ...
        /// <summary>
        /// 获取相邻指定元素之间的索引差(比如：列表中相邻两个1之间的索引差)
        /// </summary>
        /// <param name="Ary">列表</param>
        /// <param name="Member">指定元素</param>
        /// <returns>没有指定元素返回null，只有一个指定元素返回值的Count=0，否则返回相邻两个元素之间的索引差</returns>
        public static List<int> GetMemXingLinIndexChaInAry(List<byte> Ary, byte Member)
        {
            List<int> IndexCha = null;

            if (Ary == null || Ary.Count < 1 || MahjongGeneralAlgorithm.GetMenberNumInAry(Ary, Member) < 1)
            {
                return IndexCha;
            }
            IndexCha = new List<int>();

            if (MahjongGeneralAlgorithm.GetMenberNumInAry(Ary, Member) == 1)
            {
                return IndexCha;
            }
        ...
        /// <param name="card"></param>
        /// <returns></returns>
        public static int GetMenberNumInAry(List<byte> cardAry, byte card)
        {
            int num = 0;
            if (cardAry == null || cardAry.Count == 0)
            {
                return num;
            }
