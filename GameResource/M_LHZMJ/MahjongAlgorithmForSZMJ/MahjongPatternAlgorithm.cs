...
namespace MahjongAlgorithmForLHZMJ
{
    ...
    public class MahjongPatternAlgorithm
    {
        ...
        public static bool ParseCards(List<byte> srcActiveCard, ref clsFixedCard srcFixedCard, ref clsParseResult parseResult)
        {
            ...
            srcFixedCard.CopyTo(ref fixedCard);
            ...
            //先检查是否是特殊胡牌：七对子和国世无双
            if (CheckIsSpecialStruct(activeCard, ref parseResult))
            {
            ...
            if (vZhi.Count == 0)
            {
                parseResult.isSuPai = true;
            }
            if(vZhi.Count == 4)
            {
                parseResult.isSiZhong = true;
            }
            //排序一遍
            vWan.Sort();
            vTong.Sort();
            ...
            //4、对每个单色牌进行解析
            clsPairTripleWrapper pairTripleWrapper = new clsPairTripleWrapper();
            ParseSingleColorCard(vWan, ref pairTripleWrapper);	        //解析万
            ParseSingleColorCard(vTong, ref pairTripleWrapper);	        //解析筒
            ParseSingleColorCard(vTiao, ref pairTripleWrapper);	        //解析条
            ParseSingleColorCard(vZhi, ref pairTripleWrapper);	        //解析字

            //5、加上定牌组，组成最终的解析视图
            parseResult.normalParse.pair = pairTripleWrapper.cPair;
            parseResult.normalParse.activeCard.AddRange(activeCard);

            if (fixedCard.fixedCard.Count > 0)
            {
                foreach (var fc in fixedCard.fixedCard)
                {
                    clsParseTriple tagTriple = new clsParseTriple();
                    tagTriple.Clear();
                    tagTriple.enTripleType = (enTripleType)((int)(fc.fixedType));
                    tagTriple.cTokenCard = fc.card;
                    tagTriple.cCardAry[0] = fc.card;
                    tagTriple.cCardAry[1] = fc.card;
                    tagTriple.cCardAry[2] = fc.card;

                    parseResult.normalParse.activeCard.AddRange(tagTriple.cCardAry);
                    if ((enFixedCardType.FixedCardType_AGang == fc.fixedType) ||
                        (enFixedCardType.FixedCardType_BGang == fc.fixedType) ||
                        (enFixedCardType.FixedCardType_MGang == fc.fixedType))
                    {
                        parseResult.normalParse.activeCard.Add(fc.card);
                    }
                    pairTripleWrapper.AddTriple(tagTriple);
                }
            }

            for (int i = 0; i < pairTripleWrapper.tagTripleList.Length; i++)
            {
                if (pairTripleWrapper.tagTripleList[i].isValid)
                {
                    clsParseTriple item = new clsParseTriple();
                    item.cTokenCard = pairTripleWrapper.tagTripleList[i].cTokenCard;
                    item.enTripleType = pairTripleWrapper.tagTripleList[i].enTripleType;
                    Array.Copy(pairTripleWrapper.tagTripleList[i].cCardAry, item.cCardAry, 3);
                    parseResult.normalParse.triplyAry.Add(item);
                }
            }
            parseResult.isValid = true;
            return true;
        ...
        /// <param name="parseResult"></param>
        /// <returns></returns>
        public static bool CheckQueYiMen(ref clsParseResult parseResult)
        {
            List<byte> color = new List<byte>();
            color.Add(0);
            color.Add(0);
