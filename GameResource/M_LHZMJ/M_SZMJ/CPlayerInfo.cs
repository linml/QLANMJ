...
namespace M_LHZMJ
{
    public class CPlayerInfo
    {
        ...
        public bool _baoting = false;
        private int _paoNum = 0;
        public int _havepeng = 0;
        public int _havegang = 0;
        public int _havehua = 0;
        public bool _outcard;
        public int _havehun = 0;
        public bool canpeng = true;
        public bool canVote;
        ...
        public byte _fanggang;
        public byte fanggang { get { return _fanggang; } set { _fanggang = value; } }
        ...
        public byte LastInvalidCard
        {
            get
            {
                ...
                List<byte> allflowercard = this._gameServer.Szmjcmvar.GetFlowerCard();
                for (int i = 0; i < handCard.Count; i++)
                {
                    if (!allflowercard.Contains(handCard[handCard.Count - (1 + i)]))
                    {
                        return handCard[handCard.Count-i-1];
                    }
                }
        ...
        public int TotalGameScore
        {
            get
            {
                if (_gameServer._tableConfig.IsGangJiuYou)
                {
                    return _playerScore.TotalScore + _playerScore.GangScore * _gameServer._tableConfig.CellScore;
                }
                else
                {
                    //杠分+胡牌分
        ...
        public void AutoOutCard()
        {
            ...
            List<byte> handCard = CopyActiveCard();
            //花牌列表
            List<byte> flowerCardAry = this._gameServer.Szmjcmvar.GetFlowerCard();
            ...
            if (_gameServer._cardPackage.leftCardNum > 14 && !_isJustPeng)
            {
                ...
                for (int i = 0; i < handCard.Count; i++)
                {
                    if ((handCard.Count > 3) && (i < handCard.Count - 3) && !_gameServer.Szmjcmvar.HunCardAry.Contains(handCard[i]))
                    {
            ...
            if (flowerCardAry.Contains(cbOutCard))
            {
                cbOutCard = LastInvalidCard;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public bool IsHaveFlowerHardInActiveCard(List<byte> flowerhard = null)
        {
            bool ishave = false;
            ...
            List<byte> cards = this.CopyActiveCard();

            MahjongGeneralAlgorithm.RemoveAryAllParam(cards, MahjongDef.gInvalidMahjongValue);

            List<byte> allflowercard = this._gameServer.Szmjcmvar.GetFlowerCard();

            for (int i = 0; i < cards.Count; i++)
            {
                if (allflowercard.Contains(cards[i]))
                {
                    return true;
                    //ishave = true;
                    //if (flowerhard != null)
                    //{
                    //    flowerhard.Add(cards[i]);
                    //}
                }
            }
            return ishave;
        ...
        /// <returns></returns>
        public bool CheckIfCanVote(byte cbOutCard, enHuCardType huType, ushort outcardchair,bool self)
        {
            ClearVote();
            ...
            if (self == true&&_gameServer._tableConfig.IfCanBaoTing)
            {
                if (PlayerCard.tingCard.Count > 0)
                {
                    bIfCanVote = true;
                    _voteRight |= MahjongDef.gVoteRightMask_Ting;
                    _gameServer.PrintLog("有听权限");
                }
            }
            else if(_gameServer._tableConfig.is4hui)
            {

                _gameServer.PrintLog("检查" + this.PlayerChair.ToString() + "号玩家投票权限");
                if (_gameServer._tableConfig.RulePeng == 0)
                {
                    //碰
                    if (_playerCard.activeCard.Find(cbOutCard, 2) && !_giveupPeng.Contains(cbOutCard) && !_gameServer.Szmjcmvar.HunCardAry.Contains(cbOutCard))
                    {
                ...
                }else if(_gameServer._tableConfig.RulePeng == 1&&canpeng)
                {
                    //碰
                    if (_playerCard.activeCard.Find(cbOutCard, 2) && !_giveupPeng.Contains(cbOutCard) && !_gameServer.Szmjcmvar.HunCardAry.Contains(cbOutCard))
                    {
                ...
                //杠
                if (_gameServer._cardPackage.leftCardNum > _gameServer._tableConfig.MaShu && _playerCard.activeCard.Find(cbOutCard, 3)&&!_gameServer.Szmjcmvar.HunCardAry.Contains(cbOutCard))
                {
            ...
            ////胡
            //if (CheckIfCanHuACard(cbOutCard) && !(_giveupHuMultiple > 0))
            //{


            //    List<byte> activeCard = CopyActiveCard();

            //    if (this._gameServer._playerAry[outcardchair].IsBanker || this._gameServer._playerAry[this.PlayerChair].IsBanker)
            //    {
            //        bIfCanVote = true;
            ...
            //                if (IsHuKaZhang(activeCard, cbOutCard) || IsHuDanDiao(activeCard, cbOutCard) || IsHuBianZhang(activeCard, cbOutCard))
            //                {
        ...
        public bool Hua()
        {
            ...
            {
                if (flowerCardAry.Contains(activeCard[i]))
                {
                    k++;
                }
                else
        ...
        /// <param name="i"></param>
        /// <returns></returns>
        public  bool CanGang(List<byte> vSrc,int i)
        {
            ...
            gang=MahjongGeneralAlgorithm.CheckIfCanTingCardArrayHaveHun(dfs, this._gameServer.Szmjcmvar.GetHunCardAry(), null, this._gameServer._tableConfig.ifCanHu7Dui);
            return gang;
        ...
        /// <param name="i"></param>
        /// <returns></returns>
        public bool CanGang2(List<byte> vSrc, byte i)
        {
            List<byte> dfs = new List<byte>(vSrc);
            bool gang = false;
            dfs.Sort();
        ...
        /// <param name="i"></param>
        /// <returns></returns>
        public bool CanGang1(List<byte> vSrc, byte i)
        {
            List<byte> dfs = new List<byte>(vSrc);
            bool gang = false;
            dfs.Sort();
        ...
        //        }
          //  }
            ...
            //2、能杠就杠
            if (_gameServer._cardPackage.leftCardNum > 20 && _playerCard.activeCard.Find(cbVoteCard, 3)&&IsTing==false&&!_gameServer.Szmjcmvar.HunCardAry.Contains(cbVoteCard))
            {
                this._voteResult = MahjongDef.gVoteResult_Gang;
                return;
            }

            if (!_playerCard.activeCard.Find(cbVoteCard, 2) || !MahjongGeneralAlgorithm.CheckVoteRight_Peng(_voteRight) && IsTing == false)
            {
                return;
            }
            ...
            */
            ...
            if (_aiOPHelper.isOp2QYS)
            {
                ...
                {
                    if (handCard.FindIndex(p => { return _aiOPHelper.QYSColor != MahjongGeneralAlgorithm.GetMahjongColor(p); }) >= 0 && IsTing == false&&!_gameServer.Szmjcmvar.HunCardAry.Contains(cbVoteCard))
                    {
                ...
                else
                {
                    ...
                    if ((this._playerCard.tingCard.Count <= 0) && (_gameServer._cardPackage.leftCardNum < randObj.Next(20, 31)))
                    {
                        ...
                        //如果碰过之后可以听牌,就去碰
                        if (MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(handCard, new List<byte>(), _gameServer._tableConfig.ifCanHu7Dui) && IsTing == false&&!_gameServer.Szmjcmvar.HunCardAry.Contains(cbVoteCard))
                        {
                            _voteResult = MahjongDef.gVoteResult_Peng;
                            return;
                        }
        ...
        private bool VoteCompareByTing(List<byte> activeCard, ref clsFixedCard fixedCard, byte voteResult, ref int vote_type_max_leftcount, ref int vote_type_max_multiple)
        {
            ...
            {
                if (MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(activeCard, tingCard, _gameServer._tableConfig.ifCanHu7Dui))
                {
                    for (int i = 0; i < tingCard.Count; i++)
                    {
                        handcard.Clear();
                        handcard.AddRange(activeCard);
                        max = AnalysisHuCardListMultiple(tingCard[i], enHuCardType.HuCardType_ZiMo, handcard, ref fixedCard);
                        if (max > maxmultiple)
                        {
                            maxmultiple = max;
                        }
            ...
            else
            {
                ...
                for (int i = 0; i < activeCard.Count; i++)
                {
                    ...
                    if (MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(activeCard, tingCard, _gameServer._tableConfig.ifCanHu7Dui))
                    {
                        ...
                        {
                            max = AnalysisHuCardListMultiple(tingCard[j], enHuCardType.HuCardType_ZiMo, handcard, ref fixedCard);
                            if (max > maxmultiple)
                            {
                                maxmultiple = max;
                            }
        ...
        {
            gangCard.Clear();
            if (_gameServer._cardPackage.leftCardNum <= _gameServer._tableConfig.MaShu)
            {
                return false;
            }
            List<byte> activeCard = CopyActiveCard();
            ...
            if (activeCard.Count > 3)
            {
                ...
                {
                    if (activeCard[i] == activeCard[i + 3] && !_gameServer.Szmjcmvar.HunCardAry.Contains(activeCard[i]))
                    {
                        if(IsTing)
                               {

                                    if (CanGang(activeCard,i))
                                    {
            ...
            if (fixed_counterpart.fixedCard.Count > 0)
            {
                foreach (var fixedCard in fixed_counterpart.fixedCard)
                {
                    if (fixedCard.fixedType == enFixedCardType.FixedCardType_Peng && !fixedCard.isGiveUpGang2Peng && activeCard.Contains(fixedCard.card))
                    {
                        if (IsTing)
                        {
                            if (CanGang2(activeCard, fixedCard.card))
                            {
                                gangCard.Add(fixedCard.card);
                            }
                        }
            ...
            if (gangCard.Count > 0)
            {
                ...
                if (_gameServer.Szmjcmvar.HunCardAry.Contains(gangCard[0]))
                {
                    return false;
                }
        ...
        public bool TingCheck()
        {
           ...
           // return MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(activeCard, _playerCard.tingCard);
            return MahjongGeneralAlgorithm.CheckIfCanTingCardArrayHaveHun(activeCard, this._gameServer.Szmjcmvar.GetHunCardAry(), _playerCard.tingCard, this._gameServer._tableConfig.ifCanHu7Dui);
        }
        ...
        /// <param name="huType"></param>
        /// <returns></returns>
        public int HandleByHu(byte huCard, enHuCardType huType)
        {
            _cbHuCard = huCard;
            _huCardType = huType;
            ...
            return AnalysisHuCardListMultiple(huCard, huType, handCard, ref fixedCard);
        }
        ...
        public void GetTingAfterOutCardAry(List<byte> outAry)
        {
            ...
            for (int i = 0; i < activeCard.Count(); i++)
            {
                ...
                if (MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(activeCard, new List<byte>(), _gameServer._tableConfig.ifCanHu7Dui))
                {
                    outAry.Add(tempCard);
                }
        ...
        public bool CheckIfCanHuACard(byte cbCard)
        {
            ...
            //if (_playerCard.tingCard.Contains(cbCard))
            //{
            //    return true;
            //}
            //7对不能胡

            if (_gameServer._tableConfig.ifCanHu7Dui&&MahjongGeneralAlgorithm.IsHuSevenPairStruct(new List<byte>(cbCard)))
            {
                return _gameServer._tableConfig.ifCanHu7Dui;
            }
            List<byte> activeCard = CopyActiveCard();
            activeCard.Add(cbCard);

            //return MahjongGeneralAlgorithm.CheckIfCanHuCardArrayHaveHun(activeCard, this._gameServer.Szmjcmvar.GetHunCardAry(), this._gameServer._tableConfig.ifCanHu7Dui);
        ...
        /// <summary>
        /// 南京麻将检查是否可以胡(小胡小于4张硬花不能胡)
        /// </summary>
        /// <param name="cbCard"></param>
        /// <returns></returns>
        public bool CheckIfCanHuForNJMJ(byte cbCard)
        {
            bool ishu = true;

            List<byte> cardAry = new List<byte>(PlayerCard.activeCard.handCard);
            cardAry.Add(cbCard);
            ...
            //if (MahjongGeneralAlgorithmLhh.IsHu13BuKao(cardAry, ref is7xgw))
            //{
            //    if (!is7xgw)
            //    {
            //        //手中有花牌不能胡//之所以外面套了两层判断，是因为十三烂的7星归位可以在有花牌的情况下胡牌
            //        for (int i = 0; i < cardAry.Count; i++)
            //        {
            //            if (!cardAry.Contains(flowerCardAry[i]))
            //            {
            //                continue;
            //            }
            //            ishu = false;
            ...
            //{
                //手中有花牌不能胡//之所以外面套了两层判断，是因为十三烂的7星归位可以在有花牌的情况下胡牌
                for (int i = 0; i < flowerCardAry.Count; i++)
                {
                    if (!cardAry.Contains(flowerCardAry[i]))
                    {
                        continue;
            ...
            //this.NjmjVar.HuTypes.Clear();
            //this.NjmjVar.DahuOrXiaohu = DaHuOrXiaoHu.Hu_UnKnown;
        ...
        public bool CheckIfCanTingAfterOutACard(byte cbCard)
        {
            ...
            {
                return MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(activeCard, new List<byte>(), _gameServer._tableConfig.ifCanHu7Dui);
            }
            return false;
        }
        ...
        /// </summary>
        /// <param name="cCard"></param>
        public void HandleByAfterBuHua(byte cCard)
        {
            if (cCard != HoldCard)
            {
                //从手中删除打出的这张牌
            ...
            //this.SzmjVar.GuoHuBuHuCard = MahjongDef.gInvalidMahjongValue;
            //this.SzmjVar.GuoPengBuPengCard = MahjongDef.gInvalidMahjongValue;
           // this.SzmjVar.OutCardAry.Add(cCard);
          //  this.SzmjVar.AddFlowerCard(cCard, this._gameServer.Szmjcmvar.GetFlowerCard());
            //this.SzmjVar.JustGangNum = 0;
            //SzmjVar.BetwenTwoOutCardOper.Add(new TwoOutCardOper(OperClass.Oper_BuHua, (byte)this.PlayerChair));




            //#region 把花牌添加进花牌列表中
            //List<byte> CardAry = this._gameServer.Szmjcmvar.GetFlowerCard();
            //if (CardAry.Contains(cCard))
            //{
            //    this.PlayerCard.flowerCard.Add(cCard);
            //}
            //#endregion
            ...
            //打完一张牌之后马上进行听牌检查
            if (!IsHaveFlowerHardInActiveCard())
            {
                TingCheck();
            }
        ...
        /// <returns></returns>
        public int AnalysisHuCardListMultiple(byte cbHuCard, enHuCardType huType, List<byte> handCard, ref clsFixedCard fixedCard)
        {
            //胡牌牌阵
            ...
            if (2 == (handCard.Count % 3))
            {
                ...
                handCard.Sort();
                clsParseResult parseResult = new clsParseResult();
                JieSuan.Clear();
                //解析牌阵
                if (MahjongPatternAlgorithm.ParseCards(handCard, ref fixedCard, ref parseResult))
                {
                    if (enHuCardType.HuCardType_GangShangHua == huType || enHuCardType.HuCardType_ZiMo == huType || enHuCardType.HuCardType_QiangGangHu==huType)
                    {
                        score = 2;
                        if (parseResult.isSuPai)
                        {
                            score += 2;
                            //打中
                            if (PlayerCard.poolCard.Contains(0x35))
                            {
                                score += 5;
                            }
                        }
                        //全胡牌 跑中
                        if (PlayerCard.tingCard.Count == 28)
                        {
                            score += 2;
                        }
                        if (parseResult.is7Pair)
                        {
                            if (_gameServer._tableConfig.ifCanHu7Dui)
                            {
                                score += 2;
                            }
                            
                        }
                        if (parseResult.isSiZhong)
                        {
                            score += 8;
                        }
                    }
                    ...
                    {
                        if (MahjongPatternAlgorithm.CheckQueYiMen(ref parseResult))
                        {
                            score = 5;
                        }
                        else
                    ...
                    //if (MahjongGeneralAlgorithm.IsHuShiSanLan(handCard))
                    //{
                    //    parseFan.BuKao = true;
                    //}

                    //if (MahjongGeneralAlgorithm.IsHuSevenPairStruct(handCard))
                    //{
                    //    parseFan.QiDui = true;
                    //    score += 2;
                    //}
        ...
        /// </summary>
        /// <returns></returns>
        public List<byte> CopyActiveCard(bool bAddHoldCard = true, byte cbCard = MahjongDef.gInvalidMahjongValue)
        {
        ...
        private int GetMeTingCardLeftCount(List<byte> cardList = null)
        {
            ...
            if (activeCard.Count % 3 == 1)
            {
                if (MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(activeCard, ting, _gameServer._tableConfig.ifCanHu7Dui))
                {
        ...
        private byte SelACardOutThenTing()
        {
            ...
            for (int i = 0; i < outAry.Count; i++)//有多个的话，就需要考察打出哪些可以胡的牌最多和听的牌最多
            {
                ...
                if (MahjongGeneralAlgorithm.CheckIfCanTingCardArrayForWHMJ(activeCard_counterpart, ting, _gameServer._tableConfig.ifCanHu7Dui))
                {
                    //听牌张数
                    leftCount = 0;
                    ...
                    for (int j = 0; j < ting.Count; j++)
                    {
                        ...
                        mul = AnalysisHuCardListMultiple(ting[j], enHuCardType.HuCardType_ZiMo, handCard, ref fixedCard);
                        if (mul > max)
                        {
                            max = mul;
                        }
