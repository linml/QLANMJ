...
using MahjongAlgorithmForHQMJ;
using M_HQMJ_GameMessage;
using GameIF;
using QL.Common;
using GameIF.Common;
namespace M_HQMJ
{
    ...
    public class GameServer : GameIF.GamePlugBaseClass
    {
        ...
        ///续局次数(可累计)
        ///</summary>>
        public byte _addNum = 1;

        public byte xujucishu = 1;
        ...
        /// 是否续局投票中
        /// </summary>
        private bool _isAddGame = false;

        private byte[] playerVote = { 2,2,2,2};

        private DateTime _startAddTime { get; set; }
        ...
        List<byte> list = new List<byte> { 0x02, 0x19, 0x31, 0x06, 0x28, 0x16, 0x25, 0x22, 0x29, 0x23, 0x09, 0x21, 0x07, 0x17, 0x02, 0x05, 0x27, 0x22, 0x14 };
        ...
        /// <summary>
        /// 是否有玩家可以选择豹听 用于断线重连豹听问题
        /// </summary>
        private bool[] baozi = new bool[4];
        ...
        /// 游戏ID
        /// </summary>
        public override int GameID { get { return HQMJConstants.WHMJ_GAMEID; } }
        /// <summary>
        /// 游戏名称
        /// </summary>
        public override string GameName { get { return HQMJConstants.GAME_NAME; } }
        /// <summary>
        /// 游戏版本号
        /// </summary>
        public override string GameVersion { get { return HQMJConstants.GAME_VERSION; } }
            ...
            if (handle == "TryDeleteRoom")
            {
                ...
                if (TableStatus.free == 0)
                {
                    ...
                    if ((!this._tableConfig.GiveBacked || this._addNum > 1) && this._tableConfig.TableCreatorPay == 3 && GameHost.TableInfo.GroupId > 0)
                    {
                        ...
                        this._tableConfig.GiveBacked = true;
                    }                    
                    #endregion
        ...
        private void RepositTableFrameSink()
        {
            ...
            _gameID = "";
            for (short i = 0; i < HQMJConstants.GAME_PLAYER; i++)
            {
                _playerAry[i].Clear();
                //每局强退重置
        ...
        private void RecordContext()
        {
            ...
            //记录桌子玩家数据
            for (ushort i = 0; i < HQMJConstants.GAME_PLAYER; i++)
            {
                var player = GameHost.PlayerDataService.GetPlayerByChairID(i);
        ...
        public override int OnBeforGameContinueResult(TablePlayer player, int result, out string errorMsg)
        {
            //if(this._tableConfig.TableCreatorPay == 1)
            //{
            //    if(player.DiamondsNum < this._tableConfig.TableCost * (this._addNum+1))
            //    {
            //        errorMsg = "钻石不足 无法续局";
            //        return -1;
            //    }
            //}
            //if(this._tableConfig.TableCreatorPay == 2)
            //{
            //    var creatorPlayer = GameHost.PlayerDataService.GetPlayerInfoByPlayerID(this._tableConfig.TableCreatorID);
            //    if(null != creatorPlayer)
            //    {
            //        if(creatorPlayer.MoneyBag.DiamondNum < this._tableConfig.TableCost * (this._addNum+1))
            //        {
            //            errorMsg = "房主钻石不足 续局失败";
            //            return -1;
            //        }
            //        this.SendGameMsg(MahjongDef.gInvalidChar, new CMD_S_AddGameNum()
            //        {
            //            gameNum = 101
            //        });
            //    }
            //}
            return base.OnBeforGameContinueResult(player, result, out errorMsg);
        }
        ...
        /// 调用gamehost.gameend, 
        /// </summary>
        private void GameEnd(Boolean isDismissRoom = false,Boolean forceEnd = false)
        {
            //if (isDismissRoom || forceEnd || this._tableConfig.isPlayEnoughGameNum(this._addNum))
            //{//桌费
                int cost = 0;
                ...
                var balance = new PlayerForFeeDataMultiple()
                {
                    ...
                    TotalGameNum = (_tableConfig.SetGameNum + 1) * 8 * this._addNum,
        ...
        private void RealGameStart()
        {
            ...
            for (ushort i = 0; i < HQMJConstants.GAME_PLAYER; i++)
            {
                ...
                {
                    ++_onlinePlayerCount;
                    _isHaveAIPlayer = player.UserID < HQMJConstants.AIPlayerID ? true : _isHaveAIPlayer;
                    ...
                    //是否AI用户
                    _playerAry[i].IsAIPlayer = player.UserID < HQMJConstants.AIPlayerID;
                    //玩家断线状态
                    ...
                    //打印日志
                    string PlayType = player.UserID < HQMJConstants.AIPlayerID ? "<AI>" : "<真人>";
            ...
            _bCtl = (0 == (temp % HQMJConstants.GAME_PLAYER) ? false : true);
            if (this._bOutScore)
            {
                _bCtl = false;
            }
            ...
            else
            {
                ...
                // this.PrintLog("宿州麻将混皮牌：" + _hunPiCard.ToString());
                // this.GameHost.WriteMessage("宿州麻将混皮牌：" + _hunPiCard.ToString());
                ...
                //通知断线玩家
                for (int m = 0; m < HQMJConstants.GAME_PLAYER; m++)
                {
        ...
        /// 玩家续局投票超时
        /// </summary>
        void voteAddGameOutTime()
        {
            this._isAddGame = false;//续局开关 关闭
            //注销定时器
            GameHost.HostTimerService.DestroyTimer(CommonDef.TimerID_OutTime);
            this.SendGameMsg(MahjongDef.gInvalidChar, new CMD_S_AddGameOutTime()
            {
            });
            this.GameHost.WriteMessage("**游戏结束 **");
            //通知平台游戏结束
            GameEnd();
        }

        //游戏结束
        private void OnEventGameEnd(ushort wChairID, bool forceEnd = false,bool isDismissRoom = false,bool needRecord = true)
        {
            AddLogTitle("游戏结束");
            AlreadyStart = false;
            ...
            int[] anGangs = { 0,0,0,0};
            int[] mingGangs = { 0,0,0,0};
            int[] buGangs = { 0,0,0,0};
            int[] dianGangs = { 0,0,0,0};
            ...
            foreach (var player in _playerAry)
            {
                ...
                if (player.TotalGameScore != 0)
                    _noOneWin = false;
                player.TotalScore += player.TotalGameScore;
            }
            if (_noOneWin)
            {
                //lianzhuang++;//黄庄 连庄次数不变
            }
                ...
                this.SaveMsg(MahjongDef.gInvalidChar, new CMD_S_Balance()
                {
                    ...
                    isPlayEnougnGameNum = (byte)(forceEnd ? 1 : this._tableConfig.isPlayEnoughGameNum(this._addNum) ? 1 : 0)
                });
            ...
            //        if (player.IsHasGang)
            //        {
            //            _bankerChar = (byte)((_bankerChar) % HQMJConstants.GAME_PLAYER);
            //            break;
            //        }
            //    }
            //}
            ...
            if (this._tableConfig.isValid)
            {
                ...
                if (_tableConfig.IsRecordScoreRoom)
                {
                    ...
                    this.GameHost.HostGameReplayHelperServices.SaveRecord(gameRecordData, p =>
                    {
                        ...
                        {
                            if (this._tableConfig.isPlayEnoughGameNum(this._addNum) && !forceEnd)
                            {
                                if ((!this._tableConfig.GiveBacked || this._addNum > 1) && this._tableConfig.TableCreatorPay == 3 && GameHost.TableInfo.GroupId > 0)
                                {
                                    GameHost.HostSettlementService.ThawUserAccount(new ThawUserAccountParam()
                            ...
                            if (!this._tableConfig.isPlayEnoughGameNum(this._addNum))
                            {
                                //发送玩家余额
                                this.UpdateClientPlayerMoney();
                            }                            
                            ...
                            if (this._tableConfig.isPlayEnoughGameNum(this._addNum))
                            {
                                //SendGameMsg(MahjongDef.gInvalidChar, new CMD_S_AddGameNum()
                                //{
                                //    gameNum = 2
                                //});
                                //this._isAddGame = true;
                                //this._startAddTime = DateTime.Now;
                                //GameHost.HostTimerService.RegTimerNonAutoHandle(CommonDef.TimerID_OutTime, 310, () =>
                                //{
                                //    voteAddGameOutTime();
                                //});
                                List<QL.Common.PlayerGameScore> player = new List<QL.Common.PlayerGameScore>();
                                foreach (var player1 in _playerAry)
                                {
                                    player.Add(new QL.Common.PlayerGameScore
                                    {
                                        GameScore = player1.TotalScore,
                                        PlayerId = player1.PlayerID
                                    });
                                }
                                GameHost.HostGameContinueService.BeginGameContinue(player.ToArray());
                                this._isAddGame = true;
                            }
                        }
                    });
                ...
                //                {
                //                    this.GameHost.WriteMessage("** 颖上麻将,金币场，游戏结束 **");
                //                    //通知平台游戏结束
                //                    GameEnd();
                //                }
                //            });
                ...
                this.GameHost.HostGameReplayHelperServices.SaveRecord(gameRecordData, p =>
                {
                    ...
                    else
                    {
                        ...
                        GameHost.HostSettlementService.PlayerForFeeMultiple(balance, p1 =>
                        {
                            ...
                            this.UpdateClientPlayerMoney();
                            //通知平台游戏结束
                            GameEnd(isDismissRoom,forceEnd);
                        });
                    }
                });
            }
            ...
            //自建房间,局数未打完,断线玩家自动准备
            if (this._tableConfig.isValid && !this._tableConfig.isPlayEnoughGameNum(this._addNum) && !forceEnd)
            {
                for (ushort i = 0; i < HQMJConstants.GAME_PLAYER; i++)
                {
        ...
        //   result:
        //     处理结果，1同意续局。-1拒绝续局
        public override int OnGameContinueResult(int result)
        {
            GameHost.SaveLog($"续局结果{result}");
            if (result == 1)
            {
                this._addNum++;
                if (this._tableConfig.TableCreatorPay == 3 && GameHost.TableInfo.GroupId > 0)
                {
                    int moneyNum = (this._tableConfig.SetGameNum + 1) * 32 * this._addNum;
                    GameHost.HostSettlementService.FrozenUserAccount(new GameIF.FrozenUserAccountParam()
                    {
                        ...
                        MoneyNum = moneyNum,
                        Mark = "霍邱麻将圈主付费房费预扣",
                    }, Rec_PreHoldPayForAdd);
                }
                else
                {
                    this._tableConfig.GiveBacked = false;
                    for (int i = 0; i < HQMJConstants.GAME_PLAYER; i++)
                    {
                        _playerAry[i].IfCanOp = true;
                    }
                    //续局后 调用空闲进入房间 初始化桌子信息 再调用游戏开始
                    this.SendGameMsg(MahjongDef.gInvalidChar, new CMD_S_ORC_TableFree()
                    {
                    }, false);
                        this.SendGameMsg(MahjongDef.gInvalidChar, new CMD_S_AddGameNum()
                    {
        ...
        public override int ReceiveMsg(ushort chairID, GameMessage cm)
        {
            ...
            if (cm.wMainCmdID == HQMJConstants.WHMJ_GAMEID)
            {
                switch (cm.wSubCmdID)
                {
                    //抓牌结束
                    case HQMJMsgID_c2s.CMD_C_HoldCardComplete:
                        {
                            HandleMsg_CMD_C_HoldCardComplete(chairID);
                            break;
                        }
                    //玩家打出牌
                    case HQMJMsgID_c2s.CMD_C_OutCard:
                        {
                    ...
                    //玩家自摸
                    case HQMJMsgID_c2s.CMD_C_ZiMo:
                        {
                            HandleMsg_CMD_C_ZiMo(chairID);
                            break;
                        }
                    //玩家投票
                    case HQMJMsgID_c2s.CMD_C_Vote:
                        {
                    ...
                    //玩家抢杠
                    case HQMJMsgID_c2s.CMD_C_QiangGang:
                        {
                    ...
                    //请求强退
                    case HQMJMsgID_c2s.CMD_C_ForceLeft:
                        {
        ...
        public override int OnUserComeIn(ushort chairID, QL.Common.TablePlayer userInfo, byte userState)
        {
            ...
            {
                Version = HQMJConstants.GAME_VERSION,
            });
            //朋友圈游戏,创建桌子场
                    ...
                    if (this._tableConfig.isValid)
                    {
                        ...
                        else
                        {
                            //if (_tableConfig.TableCreatorPay  > 1 && userInfo.DiamondsNum < _tableConfig.TableCost && _tableConfig.GameNum == 0)
                                //NoMoneyForce(chairID, _tableConfig.TableCost);
                            //else
                            {
                                this.GameHost.WriteMessage(userInfo.NickName + "(" + userInfo.PlayerID + ")进入,此时桌子规则有效,无需创建桌子规则");
        ...
        {
            if ((!this._tableConfig.GiveBacked || this._addNum > 1)&& this._tableConfig.TableCreatorPay == 3 && GameHost.TableInfo.GroupId > 0 && _tableConfig.TableCreatorID == userInfo.PlayerID)
            {
                GameHost.HostSettlementService.ThawUserAccount(new ThawUserAccountParam()
                {
                    OrderId = quanZhuID,
        ...
        private void HandleMsg_CMD_C_Pao(ushort chair, byte point)
        {
            ...
            if (++_completeCount == HQMJConstants.GAME_PLAYER)
            {
        ...
        /// <param name="chair"></param>
        /// <param name="point"></param>
        private void HandleMsg_CMD_C_AddGameNum(ushort chair, byte point)
        {          
            if(point == 2)
            {
                PrintLog("钻石不足 无法续局 游戏结束");
                this.SendGameMsg(MahjongDef.gInvalidChar, new CMD_S_AddGameNum()
                {
                    gameNum = 101
                });
            ...
            if (point == 0)
            {
                PrintLog(chair + "号玩家同意续局");
                //this.playerVote[chair] = 0;
                //this.SendGameMsg(MahjongDef.gInvalidChar, new CMD_S_AddGameNum()
                //{
                //    gameNum = 0,
                //    chair = (byte)chair
        ...
        private void HandleMsg_CMD_C_SaveTable(ushort chair)
        {
            ...
            if (!_isSavingTable && !_isSaveTable)//
            {
                ...
                //(uint)(this._tableConfig.TableCost), "霍邱麻将房费暂扣" + _tableConfig.TableCode, p =>
                //{
                //    if (p.IsSuccess)
                //    {
                //        this.UpdateClientPlayerMoney();
        ...
        private void HandleMsg_CMD_C_NextGame(ushort chair, CMD_C_NextGame readyUser)
        {
            ...
            {
                _playerAry[chair].IfCanOp = false;
                if(!this._tableConfig.isPlayEnoughGameNum(this._addNum - 1) && !this._isAddGame)
                    _playerAry[chair].IsReady = true;
                CMD_S_UseReady alaredy = new CMD_S_UseReady();
                alaredy.chair = (byte)chair;
                //if(alaredy.userReady.Length == 0)
                //{
                //    for(var i = 0; i < 4; i++)
                //    {
                //        alaredy.userReady[i] = false;
                //    }
                //}                      
                //alaredy.userReady[chair] = true;
                if (++_completeCount >= HQMJConstants.GAME_PLAYER)//如果所有玩家可以投票
                {
                    AddLogTitle("开始游戏");
                    RealGameStart();
                }
        ...
        //    return tempgang;
        //}
        //private byte FangGangScore(int i, ushort chair)
        //{
        //    if (i == 0)
        //    {
        //        return _playerAry[chair]._fanggang1;
        //    }
        //    else if (i == 1)
        //    {
        //        return _playerAry[chair]._fanggang2;
        //    }
        //    else if (i == 2)
        //    {
        //        return _playerAry[chair]._fanggang3;
        //    }
        //    else if (i == 3)
        //    {
        //        return _playerAry[chair]._fanggang4;
        //    }
        //    return 0;
        ...
        private void HandleMsg_CMD_C_ZiMo(ushort chair)
        {
            ...
            _noOneWin = false;
            //天胡
            Boolean _tianHu = multiple[3] == 1;
            if (_tianHu)
            {
                multiple[0] -= 1;//天胡时 没有底分
            }
            //加番
            int tempjia = 2;
            if (!_tianHu && _baoTing[chair])
            {//天胡时 不算豹听
                tempjia *= 2;
            }
            ...
            for (int i = 0; i < HQMJConstants.GAME_PLAYER; i++)
            {
                ...
                {                  
                    if (_tianHu)
                    {                      
                        _playerAry[i].PlayerScore.huScore -= 10;
                        _playerAry[chair].PlayerScore.huScore += 10;
                    }
                    _playerAry[i].PlayerScore.huScore -= multiple[0] * _tableConfig.CellScore * tempjia;
                    _playerAry[chair].PlayerScore.huScore += multiple[0]* _tableConfig.CellScore * tempjia;
                }
            }        
        ...
        private void HandleMsg_CMD_C_Vote(ushort chair, byte voteResult)
        {
            ...
            {
                for (int i = 1; i < HQMJConstants.GAME_PLAYER; i++)
                {
                    int temp = (i + _outCardInfo.chair) % HQMJConstants.GAME_PLAYER;
                    if (MahjongDef.gVoteResult_Hu == _playerAry[temp].VoteResult)
                    {
                        //   huPlayer.Add(_playerAry[temp].PlayerChair);
                        this.PlayerHuCard(_playerAry[temp].PlayerChair, _outCardInfo.card, _playerAry[temp].PlayerCard.activeCard.handCard,outMj[temp],1);
                        break;
                    }
                }
                this._gamePhase = enGamePhase.GamePhase_Unknown;
                ...
                if (0 == _completeCount)//所有可以投票的玩家都已经投完票
            {
                ...
                if (!_tableConfig.YiPaoDuoXiang)
                {
                    ...
                    for (int i = 1; i < HQMJConstants.GAME_PLAYER; i++) //不带一炮多响，从出牌者的下家开始找起
                    {
                        int temp = (i + _outCardInfo.chair) % HQMJConstants.GAME_PLAYER;
                        if (MahjongDef.gVoteResult_Hu == _playerAry[temp].VoteResult)
                        {
                            _index ++;
                            huPlayer.Add(_playerAry[temp].PlayerChair);
                            this.PlayerHuCard(_playerAry[temp].PlayerChair, _outCardInfo.card, _playerAry[temp].PlayerCard.activeCard.handCard,outMj[_playerAry[temp].PlayerChair],_index);
                            break;
                        }
                    }
                }
        ...
        /// <param name="chair"></param>
        /// <param name="card"></param>
        private void PlayerHuCard(ushort chair, byte card, List<byte> list,int countOutMj,int isFirst = 0)
        {
            _playerAry[chair].IfCanOp = false;
            _outCardRecord.Add(card);
            ...
            //else
            //{
                multiple = _playerAry[chair].HandleByHu((byte)chair,card, enHuCardType.HuCardType_PingHu, list, countOutMj, isFirst);
            //}
            ...
            //if (_tableConfig.zhanZhuang == 1 && tempBanker == _outCardInfo.chair && lianzhuang > 1)
            //{//庄家点炮 单独输庄分
            //    tempScore[_outCardInfo.chair] -= (lianzhuang - 1) * 2 * tempjia;
            //    tempScore[chair] += (lianzhuang - 1) * 2 * tempjia;
            //    _playerAry[_outCardInfo.chair].VecType += "庄分- "+(lianzhuang-1);
            //} 
            //if(_tableConfig.zhanZhuang == 1 && tempBanker == chair && lianzhuang > 1)
            //{//庄家胡牌 其他三家输庄分(或者点炮玩家输庄分)
            //    if(_tableConfig.whoLose == 1)
            //    {
            //        tempScore[chair] += (lianzhuang - 1) * 2 * tempjia;
            //        tempScore[_outCardInfo.chair] -= (lianzhuang - 1) * 2 * tempjia;
            //    }
            ...
            //                tempScore[i] -= (lianzhuang - 1)*2 * tempjia;
            //            if (i != tempBanker && i != _outCardInfo.chair)
            //                tempScore[i] -= (lianzhuang - 1) * tempjia;      
            //        }
            //        tempScore[chair] += (lianzhuang - 1) * 4;
            //    }
            //    _playerAry[chair].VecType += "占庄+ " + (lianzhuang - 1);
        ...
        private void HandleMsg_CMD_C_QiangGang(ushort chair, byte qiangGang)
        {
            ...
            if (_completeCount == _waitQiangGangPlayer.Count)
            {
                ...
                if (_tableConfig.YiPaoDuoXiang)
                {
                    int huPerson = 0;
                    int _index = 0;
                    for (int j = 1; j < HQMJConstants.GAME_PLAYER; j++)
                    {
                        int temp = (j + _qiangGangInfo.chair) % HQMJConstants.GAME_PLAYER;
                        {
                            if (_playerAry[temp].IsVoteQiangGang)
                            {
                                ...
                                int[] multiple = _playerAry[temp].HandleByHu((byte)temp, _qiangGangInfo.card,
                                    enHuCardType.HuCardType_QiangGangHu, _playerAry[temp].PlayerCard.activeCard.handCard,outMj[temp],_index);
                                _noOneWin = false;
                                int[] tempScore = new int[4];
                                ...
                                {//清一色包分(第一个抢杠胡的才能包分)
                                    if (huPerson == 1)
                                    {
                                        ...
                                        for (int i = 0; i < HQMJConstants.GAME_PLAYER; i++)
                                        {
                                            ...
                                            {
                                                _playerAry[i].PlayerScore.huScore -= multiple[0] * 4 * tempjia;
                                        ...
                                        _playerAry[temp].PlayerScore.huScore -= _playerAry[_qiangGangInfo.chair].PlayerScore.huScore;
                ...
                {
                    for (int j = 1; j < HQMJConstants.GAME_PLAYER; j++)//不带一炮多响，从出牌者的下家开始找起
                    {
                        int temp = (j + _qiangGangInfo.chair) % HQMJConstants.GAME_PLAYER;
                        if (_playerAry[temp].IsVoteQiangGang)
                        {
                            lastHuChair = (ushort)temp;
                            _playerAry[_qiangGangInfo.chair].IsFangPao = true;
                            int[] multiple = _playerAry[temp].HandleByHu((byte)_qiangGangInfo.chair, _qiangGangInfo.card, enHuCardType.HuCardType_QiangGangHu, _playerAry[_qiangGangInfo.chair].PlayerCard.activeCard.handCard, outMj[temp],1);
                            _noOneWin = false;
        ...
        private void HandleMsg_CMD_C_CreateTable(ushort chair, CMD_C_CreateTable tableRule)
        {
            ...
            //else
            {
                ...
                _tableConfig.CheckGps = tableRule.IfCheckGps > 0 ? true : false;
                ...
                _tableConfig.gangFen = tableRule.gangFen;
                ...
                //if (player.MoneyBag.DiamondNum < 101)
                //    NoMoneyForce(chair, _tableConfig.TableCost);
            }
        }
        ...
        void Rec_PreHoldPayForAdd(FrozenUserAccountResult result)
        {
            if (result.IsSuccess)
            {
                this._tableConfig.GiveBacked = false;
            ...
            if (this._tableConfig.isValid && this._dissTable.isValid && (chair != this._dissTable.SponsorChair))
            {
                ...
                else
                {
                    ...
                    if (this._dissTable.playerVote((byte)chair, dissTableVote.voteResult))
                    {
                        ...
                        {
                            this.OnEventGameEnd(MahjongDef.gInvalidChar, true, true,false);
        ...
        private bool SendCardToPlayer(bool isUsual = true)
        {
            ...
            if (NeedOverGame)
            {
                ...
                GameHost.HostTimerService.RegTimerNonAutoHandle(CommonDef.TimerID_WaitGoon, 10, () =>
                {
                    ...
                    //            for (int j = 0; j < HQMJConstants.GAME_PLAYER; j++)
                    //            {
                    //                if (i != j)
                    //                {
                    //                    _playerAry[i].PlayerScore.huScore += FangGangScore(j, (ushort)i);
                    //                    _playerAry[j].PlayerScore.huScore -= FangGangScore(j, (ushort)i);
                    //                }
            ...
            //如果是第一次抓牌 判断是否已经听牌
            if(outMj[_opPlayerChar] == 0 && tingMj[_opPlayerChar] && _playerAry[_opPlayerChar].PlayerCard.fixedCard.fixedCard.Count == 0)
            {//玩家可选择豹听
                CMD_S_PlayerBaoTing baoting = new CMD_S_PlayerBaoTing();
                baoting.chair = _opPlayerChar;
                SendGameMsg(_opPlayerChar, baoting);
                baozi[_opPlayerChar] = true;
                return false;
            }
            //累计抓牌的数量
            if (isUsual)
            ...
            {
                //if (_playerAry[_opPlayerChar].IsJustGang)
                //{
                //    holdCard = _hunPiCard;
        ...
        private void SetSZInfo()
        {
            ...
            else
            {
                ...
                //else
                //{
                //    // _bankerChar = (byte)((_bankerChar + 1) % HQMJConstants.GAME_PLAYER);
                //    szInfo.bankerChair = _bankerChar;
        ...
        private void InitPlayerCard()
        {
            ...
            for (short i = 0; i < HQMJConstants.GAME_PLAYER; i++)
            {
                ...
                if (i == 0)//_playerAry[i].IsAIPlayer)
                {
                    ...
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x31);
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x31);
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x29);
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x29);
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x29);
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x09);
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x09);
                    _playerAry[i].PlayerCard.activeCard.handCard.Add(0x09);
        ...
        private void GetNextOpPlayer(byte checkChair)
        {
            ...
            do
            {
                checkChair = (byte)((checkChair + 1) % HQMJConstants.GAME_PLAYER);
        ...
        /// 玩家余额不足,强退玩家
        /// </summary>
        private void NoMoneyForce(ushort chairID,int cost)
        {
            ...
            //GameHost.PlayerDataService.PostUserExitMessage(GameHost.PlayerInfoOnTableSeat[chairID], string.Format("抱歉，您的钻石不足！进入此房间需要{0}张房卡。", _tableConfig.TableCost));
            //this.SendGameMsg(chairID, new CMD_S_ForceUserLeft()
            //{
            //    cost = cost
            //}, false);
        ...
        private bool changeTableCreator()
        {
            ...
            do
            {
                tableCreator = (byte)((tableCreator + 1) % HQMJConstants.GAME_PLAYER);
        ...
        private void HandlerPlayerOffline(byte chairID)
        {
            ...
            //断线重连自主建房,局数没打满处理
            //  else
            if (_tableConfig.isValid && !this._tableConfig.isPlayEnoughGameNum(this._addNum) && !_isAddGame && !this._isGameing)
            {
        ...
        private void HandlerPlayerOfflineCome(byte chairID)
        {
            ...
            for (byte i = 0; i < 4; i++)
            {
                if (baozi[i])
                {
                    this.SendGameMsg(i, new CMD_S_ORC_BaoTing()
                    {
                        chair = i
                    }, false);
                }
            ...
            //断线重连 续局投票
            //if (_tableConfig.isValid && !this._isGameing && this._isAddGame)
            //{
            //    //int[] tempScore = new int[4];
            //    //for(var i = 0; i < 4; i++)
            //    //{
            //    //    tempScore[i] = _playerAry[i].TotalScore;
            //    //}
            //    this.SendGameMsg(chairID, new CMD_S_ORC_AddGameNum()
            //    {
            //        chair = chairID,
            //        //tempScore = tempScore,
            //        //playerVote = this.playerVote,
            //        //leftTime = (byte)(30 - MahjongGeneralAlgorithm.GetSecondsInTwoTime(this._startAddTime, DateTime.Now))
            //    });
            //}
        ...
        private void playerScoreChange(byte chairID)
        {
            ...
            if (_tableConfig.isValid)
            {
                ...
                {
                    if (pgRecord.playerRecord.Count == HQMJConstants.GAME_PLAYER)
                    {
        ...
        private void SendTableRule2Client(ushort chair)
        {
            ...
            this.SendGameMsg(chair, new CMD_S_TableConfig()
            {
                ...
                tableWhere = (int)GameHost.TableInfo.GroupId,
        ...
        public byte BankerDownChair
        {
            get
            {
                return (byte)((_bankerChar + 1) % HQMJConstants.GAME_PLAYER);
            }
        }
        /// <summary>
        /// 庄家对家玩家的椅子号
        ...
        public byte BankerOppoChair
        {
            get
            {
                return (byte)((_bankerChar + 2) % HQMJConstants.GAME_PLAYER);
            }
        }
        /// <summary>
        /// 庄家上家或是左手边玩家的椅子号
        ...
        public byte BankerUpChair
        {
            get
            {
                return (byte)((_bankerChar + 3) % HQMJConstants.GAME_PLAYER);
            }
        }
        ...
        {
            if (charID < HQMJConstants.GAME_PLAYER)
            {
