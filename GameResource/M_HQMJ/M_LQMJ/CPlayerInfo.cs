...
namespace M_HQMJ
{
    public class CPlayerInfo
    {
        ...
        /// <returns></returns>
        public bool CheckIfCanVote(byte cbOutCard, enHuCardType huType, ushort outcardchair, bool canChi, bool isBaoTing)
        {
            ClearVote();
            if (MahjongDef.gInvalidMahjongValue == cbOutCard)
            {
        ...
        public int[] AnalysisHuCardListMultiple(byte chair, byte cbHuCard, enHuCardType huType, List<byte> handCard, ref clsFixedCard fixedCard, List<byte> list, int countOutMj,int isFirst = 0)
        {
            ...
            if (2 == (handCard.Count % 3))
            {
                ...
                {//有吃牌 不能为对对胡
                    ...
                    {
                        if (fixedCard.fixedCard[i].fixedType == MahjongAlgorithmForHQMJ.enFixedCardType.FixedCardType_Chi)
                        {
                            duiduihu = false;
                        }
                    }
                ...
                if (fixedCard.fixedCard.Count != 0)
                {
                    for (int i = 0; i < fixedCard.fixedCard.Count; i++)
                    {
                        if (fixedCard.fixedCard[i].fixedType == MahjongAlgorithmForHQMJ.enFixedCardType.FixedCardType_MGang
                            || fixedCard.fixedCard[i].fixedType == MahjongAlgorithmForHQMJ.enFixedCardType.FixedCardType_BGang
                            || fixedCard.fixedCard[i].fixedType == MahjongAlgorithmForHQMJ.enFixedCardType.FixedCardType_AGang)
                        {//杠牌加一张 不算四归一
                            listCopy.Add(fixedCard.fixedCard[i].card);
                            pengGang++;
                        }
                        if (fixedCard.fixedCard[i].fixedType == MahjongAlgorithmForHQMJ.enFixedCardType.FixedCardType_Peng)
                        {//碰牌加三张 可能形成四归一
                            listCopy.Add(fixedCard.fixedCard[i].card);
                            listCopy.Add(fixedCard.fixedCard[i].card);
                            listCopy.Add(fixedCard.fixedCard[i].card);
                ...
                if (_gameServer._tableConfig.daiDaPai == 1 && (countWan == listCopy.Count || countTong == listCopy.Count || countTiao == listCopy.Count))
                {//清一色
                    score += 5;
                ...
                {
                    if (_gameServer._tableConfig.daiDaPai == 1 && countZhi != 0 && (countWan + countTong == 0 || countWan + countTiao == 0 || countTong + countTiao
                         == 0))
                    {//混一色
                        score += 3;
