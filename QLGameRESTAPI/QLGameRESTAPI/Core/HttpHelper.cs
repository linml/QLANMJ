...
namespace QLGameRESTAPI.Core
{
    ...
    public static partial class HttpHelper2
    {
            ...
            if (!string.IsNullOrEmpty(certPath) && File.Exists(certPath))
            {
                ...
                var ext = Path.GetExtension(certPath);
                {
                    switch (ext.ToLower())
                    {
                        case "hccert":
                            {
                                var base64 = FileHelper.ReadAllText(certPath);
                                var bytes = QL.Serialization.Base64Serialzier.FromBase64String(base64);
        ...
        {
            return GetStringFromResponseByEncoding(httpResponse, baseEncoding);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <returns></returns>
        public static string GetStringFromResponseByEncoding(this WebResponse httpResponse, Encoding encoding)
        {
            try
            {
        ...
        static void BeginPostFromData(string url, IWebParamData _query, PostFromDataParam<PostFileData> files, Action<HttpWebResponse, Exception> callback)
        {
            QL.Threading.ThreadPoolProviderManager.QueueUserWorkItem(() =>
            {
                try
                {
        ...
        /// <param name="files"></param>
        /// <param name="callback"></param>
        public static void BeginPostFromDataString(string url, IWebParamData _query, PostFromDataParam<PostFileData> files, Action<string, Exception> callback)
        {
        ...
        private static void Process_HttpPostAsync(HttpWebRequest req, Stream ms, Action<HttpWebResponse, Exception> callback)
        {
            try
            {
        ...
        private static void Process_HttpAsync(HttpWebRequest req, Action<HttpWebResponse, Exception> callback)
        {
            try
            {
        ...
        private static void Process_GetHttpResponseString(HttpWebResponse res, Exception ex, Action<string, Exception> callback)
        {
            if (ex != null)
            {
                callback?.Invoke("", ex);
