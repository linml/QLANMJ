...
$menu_id = $menu_name = $menu_url = $module_id = $is_show =$online = $menu_type = $shortcut_allowed = $menu_desc = $father_menu = '';
	...
	if($menu_name == "" || $menu_url =="" || ($menu_id>100 && empty($module_id)) ){
		...
		if(!$exist){
			$update_data = array ('menu_name' => $menu_name, 'menu_url' => $menu_url,  
								'is_show' => $is_show, "online" => $online,'menu_type'=>$menu_type,'menu_desc' => $menu_desc, 'shortcut_allowed' => $shortcut_allowed,
...
$type_options_list=array("1"=>"管理后台","2"=>"代理后台");
...
Template::display ( 'acepanel/menu_modify.tpl' );