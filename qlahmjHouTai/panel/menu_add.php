...
$menu_name = $menu_url = $module_id = $is_show = $online = $menu_type = $shortcut_allowed = $menu_desc = $father_menu = '';
	...
	if($menu_name=="" || $menu_url == ""|| $module_id == ""){
		...
		if(!empty($menu)){
			...
			$input_data = array ('menu_name' => $menu_name, 'menu_url' => $menu_url, 'module_id' => $module_id, 
								'is_show' => $is_show, 'online' =>1 , 'menu_type'=>$menu_type,'menu_desc' => $menu_desc ,'shortcut_allowed'=>$shortcut_allowed,'father_menu'=>$father_menu );
...
Template::display ( 'acepanel/menu_add.tpl' );