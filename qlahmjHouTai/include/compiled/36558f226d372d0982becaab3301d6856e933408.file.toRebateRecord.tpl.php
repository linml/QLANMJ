<?php /* Smarty version Smarty-3.1.15, created on 2018-08-29 10:03:46
      ...
      $(document).on('click','#dateSure',function(){
        ...
        $.ajax({
          ...
          success:function(data){
            ...
            $('#expend').text(-Math.round(incomeExpend['expend']))
            $('#income').text(Math.round(incomeExpend['income']))             
          },
          error : function(){
            console.log('error')
          }
    ...
    function dealData(recordData,arrLen){
      ...
      for(var i = 0; i < arrLen; i++){
                            ...
                            result +=  '<li class="relative C_wh bgc_fff"><div class="S_wh round bgc_red f_get absolute top15 left5">支</div><div class="C_wh80 floatR B_b_bfbfbf"><div class="floatL"><p class="f_shop">余额进'+Math.round(recordData[i].diamond)+'钻</p><p class="f_return" style="text-align: left">[代理进钻]</p></div><div class="floatR"><input class="f_money" style="position: absolute;left:40%;top:10%; color:red;background-color: transparent;" value="'+Math.round(recordData[i].fundmoney)+'" disabled><p class="f_return" style="position: absolute;left:64%;top:64%;">'+recordData[i].addtime+'</p></div></div></li>';
                                ...
                                result += '<li class="relative C_wh bgc_fff" ><div class="S_wh round bgc_0e932e f_get absolute top15 left5">收</div><div class="C_wh80 floatR B_b_bfbfbf"><div class="floatL"><p class="f_shop">[玩家<span>'+recordData[i].userid+'</span>]消费<span>'+Math.round(recordData[i].paynum)+'</span>元</p>'+returnlevel+'</div><div class="floatR"><input class="f_money" style="position: absolute;left:40%;top:10%;background-color: transparent;color:#0e932e" value="'+Math.round(recordData[i].fundmoney)+'" disabled><p class="f_return" style="position: absolute;left:64%;top:64%;">'+recordData[i].addtime+'</p></div></div></li>';
                            }
      }
      return result
    }
