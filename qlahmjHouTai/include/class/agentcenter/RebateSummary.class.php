...
class RebateSummary extends Base {
	...
	public static function getRebate($agentid = '',$type = '',$start = '',$page_size = ''){
		...
		if($type == 1){
			$sql = 'select dateid,dayreturn as rebate from '.self::$agent_stat_day.' where agentid = '.$agentid.' order by dateid desc '.$limit;
		}elseif($type == 2){
			$sql = 'select DISTINCT(DATE_FORMAT(dateid,"%Y-%m")) as dateid,sum(dayreturn) as rebate from '.self::$agent_stat_day.' where agentid = '.$agentid.' group by DATE_FORMAT(dateid,"%Y-%m") order by dateid desc '. $limit;
		}
	...
	public static function getRebateCountsByAgentId($agentid='',$type=''){
		...
		if($type == 1){
			...
			$sql = 'select count(DISTINCT(DATE_FORMAT(dateid,"%Y-%m"))) as counts from '.self::$agent_stat_day.' where agentid = '.$agentid;
		}
