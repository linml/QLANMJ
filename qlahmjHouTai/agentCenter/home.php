...
$dateid     = array_reverse(array_column($getDayActiveData, 'dateid'));
$dateidArr = array();
foreach ($dateid as $key => $value) {
	array_push($dateidArr,substr($value, 5));
}
$activeData = array_reverse(array_column($getDayActiveData, 'dayreturn'));
$returnData = array_reverse(array_column($getDayReturnData, 'dayreturn'));
