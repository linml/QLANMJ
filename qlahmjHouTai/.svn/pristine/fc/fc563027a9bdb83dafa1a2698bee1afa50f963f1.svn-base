...
$method = $page_no = $userid  = $agentid = $status = $agentname = $agentid = $diamond = $money = $type = $remark = $level = $where = $adminid = $mobilenum = '' ;
...
if($method == 'changeAdmin'&&!empty($agentid)&&!empty($adminid)){
	$result = Agent::resetAdminid($agentid,$adminid);
	exit(json_encode($result));
}
...
if($method == 'changeDirAgent'&&!empty($userid)){
	$result = Agent::changeDirAgent($userid,$agentid);
	exit(json_encode($result));
}
if($method == 'changeAccount'&&!empty($userid)&&!empty($mobilenum)){
	$result = Agent::changeAccount($userid,$mobilenum);
	exit(json_encode($result));
}
...
Template::assign ( 'showInfoAdminList', json_encode($admin) );
