...
class Common {
	...
	/**
	 * [checkNoNeedLogin 检测action_url是否存]
	 * @Author   李爽
	 * @DateTime 2018-07-26T14:36:00+0800
	 * @param    [type]                   $action_url          [验证url]
	 * @param    [type]                   $no_need_login_array [验证数组]
	...
	public static function getDateTime($time = null) {
		return (!is_numeric($time)) ? null : date( 'Y-m-d H:i:s', $time);
	}
