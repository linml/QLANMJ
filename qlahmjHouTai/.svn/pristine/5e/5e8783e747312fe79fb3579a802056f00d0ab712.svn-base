 	...
 	public static function getDayActive(){
 		...
 		$db = self::__instance();
			$sql = "SELECT dd.dateid,ifnull(utd.roomcnt,0) as dayreturn
			        ...
			        left join 
			        ( SELECT DATE_FORMAT(os.addtime,'%Y-%m-%d') AS dateid,COUNT(os.setid) AS roomcnt
			      FROM order_settlement os,order_settlement_detail osd 
			      WHERE os.setid=osd.setid
			        AND os.groupid IN (SELECT fi.friendid from friend_info fi WHERE fi.userid=".$userid.")
			        AND os.addtime>= date_sub(curdate(), interval 8 day)
			     GROUP BY DATE_FORMAT(os.addtime,'%Y-%m-%d')) utd 
