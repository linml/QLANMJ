...
@ccclass
export default class M_HQMJVoice{
    ...
    public static PlayChatVoice(value: number, gender: number,Type:VoiceType) {
        ...
        HQMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }else{
        ...
        // var path = "resources/gameres/M_HQMJ/sound/";
        // if (gender == 1) {
    ...
    public static PlayCardType(str:string) {
        var path = HQMJSoundDef.VoicePath;
        // if (gender == 1) {
    ...
    public static PlayPublicSound(value: string) {
        var path = cc.url.raw(HQMJSoundDef.CommonVoicePath + "Voice/" + value + ".mp3");
        HQMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放系统音效
    ...
    public static PlaySysSound(value: string) {
        var path = cc.url.raw(HQMJSoundDef.VoicePath + "SysSound/Sys76_" + value + ".mp3");
        HQMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放bgm
