...
@ccclass
export default class HQMJ_ActiveBase extends HQMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<HQMJ_SingleActiveBase>;
    /**
     * 手牌
     * */
    ...
    public init(logicChair: number):void{
        ...
        this._cardData = new Array<HQMJ_SingleActiveBase>(0);
    ...
    public get isHoldAfter():boolean{
        return this._holdCard != HQMJMahjongDef.gInvalidMahjongValue;
    }
    ...
    public refreshHandCardData(cardAry:Array<number>):void{
        ...
        if(!HQMJMahjongAlgorithm.IsSame(cardAry,this._handCard))
        {
    ...
    public outACard(card: number): void {
        ...
        if(card==this._holdCard){
            ...
            _hqmj.getFreeActive(this._logicChair).put(this._cardData.pop().node);
    ...
    private delCard(cardAry:Array<number>):void{
        ...
        HQMJMahjongAlgorithm.delCard(this._handCard,cardAry);
    ...
    public showLieCard(cardAry:Array<number>,huCard:number):void{
        ...
        if(HQMJMahjongDef.gInvalidMahjongValue != huCard){
    ...
    public playerHu(huCard: number):void{
        ...
        //如果抓到的牌不为空,那么就是自摸
        if(HQMJMahjongDef.gInvalidMahjongValue != this._holdCard){
    ...
    protected handCardChange(): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(let i: number = 0;i < newNum;i++) {
                let newnode = _hqmj.getFreeActive(this._logicChair).get();
            ...
            for(let i: number = 0;i < delNum;i++) {
                ...
                _hqmj.getFreeActive(this._logicChair).put(this._cardData[i].node);
            }
    ...
    public DelNullData():void{
        ...
        while(this._cardData.length > 0) {
            ...
            let active: HQMJ_SingleActiveBase = this._cardData.shift();
