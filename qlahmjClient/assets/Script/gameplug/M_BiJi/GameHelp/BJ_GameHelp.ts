...
import GameLogic from "./BJ_GameLogic";
 ...
 * 玩家人数
 */
export const PlayerCount: number = 5;
/**
 * 牌数
 */
export const CardsCount: number = 9;
/**
 * 最后一张牌索引
 */
 ...
 * 预设体路径
 */
export const PrefabPath: string = "gameres/M_BiJi/Prefabs/";
/**
 * 字体路径
 */
 ...
 * 音频路径
 */
export const VoicePath: string = "resources/gameres/M_BiJi/Voice/";
/**
 * 公共音频路径
 */
...
export class ScoreView {
     ...
     * 计分数据
     */
    public headscorelist: ScoreEle[];
    /**
     * 计分数据
     */
    public opposcorelist: ScoreEle[];
    /**
     * 计分数据
     */
    public lastscorelist: ScoreEle[];
        /**
     * 计分数据
     */
    public pokerlist:number[][];
    public xifendata:number[][];
    public winhead:number[];
    public winoppo:number[];
    public winlast:number[];
    public allwin:number[];
    public xifentime:number[];
    /**
     * 局号
     */
    public gameNum: string;

    public constructor(clist: number[],poker:number[][],xifenresult:number[][], _IDlist: number[], nlist: string[], flist: string[], gNum: string = "") {
        ...
        for(var i = 0;i<5;i++){
            if(poker[i].length<=0){
                this.pokerlist = new Array(i);  
                this.xifendata = new Array(i);
                break;
            }
            if(i==4){
                this.pokerlist = new Array(5);
                this.xifendata = new Array(5);
                break;
            }
        }
        ...
        for(var i = 0;i<this.pokerlist.length;i++){
            ...
            this.pokerlist[i] = new Array(poker[i].length);
            this.xifendata[i] = new Array(xifenresult[i].length);
            GameLogic.CopyArray(this.pokerlist[i],poker[i],poker[i].length);
            GameLogic.CopyArray(this.xifendata[i],xifenresult[i],xifenresult[i].length);
        ...
        this.headscorelist = new Array(0);
        this.opposcorelist = new Array(0);
        this.lastscorelist = new Array(0);
    //    this.pokerlist = [5][];
        this.gameNum = gNum;
    }
    /**
    ...
    public AddheadScoreEle(value: ScoreEle) {
        this.headscorelist.push(value);
    }
    public AddoppoScoreEle(value: ScoreEle) {
        this.opposcorelist.push(value);
    }
    public AddlastScoreEle(value: ScoreEle) {
        this.lastscorelist.push(value);
    }
    public SetJieSuanScoreEle(head:number[],oppo:number[],last:number[],allwin:number[],xifen:number[]){
        this.winhead = new Array(head.length);
        this.winoppo = new Array(oppo.length);
        this.winlast = new Array(last.length);
        this.allwin = new Array(allwin.length);
        this.xifentime = new Array(xifen.length);
        this.winhead = head;
        this.winoppo = oppo;
        this.winlast = last;
        this.allwin = allwin;
        this.xifentime = xifen;

    }
    // public AddPokerEle(value:number[][],chair:number){
    //     for(var i = 0;i<va)
    // }
}
...
public havedropCard:boolean;

public havexiScore:boolean;
...
export class TableInfo {
    ...
    */
    ...
    public Init() {
        ...
        this.havedropcard = false;
        this.havexiscore = false;
        this.havesanshunzi = false;
        this.Reset();
    }
    public Reset() {
    ...
    public SetHaveDropCard(value:boolean){this.havedropcard = value;}
    public SetHaveXiScore(value:boolean){this.havexiscore = value;}
    public SetHaveSanShunZi(value:boolean){this.havesanshunzi = value;}
