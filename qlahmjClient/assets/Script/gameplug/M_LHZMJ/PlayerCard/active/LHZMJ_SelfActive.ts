...
import LHZMJ_SelfSingleActive from "../single/active/LHZMJ_SelfSingleActive";
...
@ccclass
export default class LHZMJ_SelfActive extends LHZMJ_ActiveBase {

    onLoad() {
        // init logic
        super.onLoad();
        this.init(0);
        this.node.on(LHZMJEvent.LHZMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    }
    ...
    public holdTricksCard(holdIdx: number): number {
        ...
        for(var i: number = 0;i < holdNum;i++) {
            this._handCard.push(LHZMJ.ins.iclass.getSelfHandCardData()[i]);//[holdIdx*4 + i]);
        }
            ...
            for(let i: number = 0;i < upCard.length;i++) {
                for(let j: number = 0;j < this._cardData.length;j++) {
                    if(upCard[i] == this._cardData[j].cardValue) {
                        (<LHZMJ_SelfSingleActive>this._cardData[j]).setUp();
                        break;
                    }                      
                }
            }
    ...
    public setUpCardDown():void{
        for(let j: number = 0;j < this._cardData.length;j++) {
            (<LHZMJ_SelfSingleActive>this._cardData[j]).setDown();
        }
    }
    /**
     * 设置听牌标志牌
    ...
    public showTingCardToken(cardAry: Array<number>): void {
        if((null == cardAry) || (cardAry.length < 1)){
            ...
            for(var i: number = 0;i < this._cardData.length;i++) {
                (<LHZMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard = false;
            }
        ...
        for(var i:number=0; i<this._cardData.length; i++){
            for(var j:number=0; j<cardAry.length; j++){
                if(this._cardData[i].cardValue == cardAry[j]){
                    (<LHZMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard=true;
                    break;
                }
            }
        }
    ...
    public refreshCardStatus(): void {
            for(var j: number = 0;j < this._cardData.length;j++) {
                (<LHZMJ_SelfSingleActive>this._cardData[j]).enable = true;
                (<LHZMJ_SelfSingleActive>this._cardData[j]).showCoverBG = false;
            }
    }
    ...
    public activeEnable(enable: boolean): void {
        for(var i:number=0; i<this._cardData.length; i++){
            (<LHZMJ_SelfSingleActive>this._cardData[i]).enable =  enable;
            if(!enable){
                (<LHZMJ_SelfSingleActive>this._cardData[i]).showCoverBG=false;
            }
        }
    }
        ...
        if(LHZMJ.ins.iclass.is2D()){
            ...
            for(var i: number = 0;i < this._cardData.length;i++) {
                ...
                if(this.isHoldAfter && (i == (this._cardData.length - 1))) {
                    ...
                    if(!this.isLie && !LHZMJ.ins.iclass.isVideo())
                    {(<LHZMJ_SelfSingleActive>this._cardData[i]).setUp();}
                }
            ...
            //起始位置
            let startPos: number = LHZMJ_SelfActive.ArrangeStartPos3D[this.fixedCardNum];
            //开始排版
    ...
    //             //newnode.on(LHZMJEvent.LHZMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
     ...
     * 单个活动牌事件
     * */
    private onSingleActiveEvent(e:LHZMJEvent):void{
        
        switch(e.msgCode){
            case LHZMJEvent.msg_reflashHandCard:{
                e.stopPropagation();
                ////cc.log("自己刷新！");
                this.refreshHandCard();
                break;
    ...
    private OutCardAfter(outindex:number):void{
        //LHZMJMahjongAlgorithm.delCard(this._handCard,[card]);
        // //cc.log("位序调整");
        // let str1="";
        ...
        // }
        // //cc.log(str1);
        let tempCard:LHZMJ_SingleActiveBase;
