import SkinCardsView from "./SkinCardsView";
import { UserState, TexturePath, GameModel, FontPath } from "../GameHelp/GameHelp";
import { NiuNiu } from "../GameHelp/INiuNiuClass";
import { LoadHeader } from "../../../Tools/Function";
import { SetTextureRes } from "../GameHelp/NiuNiuFunction";
import SkinAniLockMaster from "../AniView/SkinAniLockMaster";
import SkinAniWinGold from "../AniView/SkinAniWinGold";
...
export default class SkinPlayer extends cc.Component {
    ...
    private img_guzhang:cc.Animation = null;
    //变量
    ...
    onLoad() {
        ...
        this.node.addChild(this.skinCardsView.node);
    ...
    public SelfReadyClear() {
        this.skinCardsView.Reset();
        this.HideUserRobRate();
        this.HideUserBetRate();
        this.HideUserSelectCards();
        this.img_frame.node.active = false;
        if (NiuNiu.ins.iview.GameModel() == GameModel.RobMaster)
            this.img_master.node.active = false;
    ...
    public SetUserInfo(faceID: string, name: string, gender: number) {
        ...
        this.img_userimg.node.width=95;
        this.img_userimg.node.height =95;
    }
    /**
     * 设置玩家余额
     */
    ...
    public SetUserRobRate(value: number) {
        if (value == 0) {
            ...
            this.group_robmaster.active = true;
            SetTextureRes(TexturePath + "niuniures/view_rob", this.img_robmaster);
     ...
     * 隐藏玩家边框
     */
    public HideUserFrame() {
        this.img_frame.node.active = false;
        if (NiuNiu.ins.iview.GameModel() != GameModel.RobMaster && NiuNiu.ins.iview.Master() == this._chair) {
    ...
    public ShowChatEmoji(value: cc.AnimationClip) {
        ...
        this.scheduleOnce(this.AniEmojiFinish, 3 * NiuNiu.ins.iclass.GetSpeed());
    }
    public ShowGuZhang(){
        this.img_guzhang.node.active = true;
        this.img_guzhang.play();
        this.scheduleOnce(this.AniGuZhangFinish, 2 * NiuNiu.ins.iclass.GetSpeed());
    }
    ...
    public OnImgClick() {
        ...
        NiuNiu.ins.iview.ShowPlayerInfo(this._chair);
    }
    public StopAni() {
